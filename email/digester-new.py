    ...
    def parse(self,fp,headersonly=True):
        ...
        for (field, wspc, content) in self.lines:
            ...
            # it must be a "normal" header field
                ...
                if not self.msg[curfield]: self.msg[curfield] = content.strip()
...
def unfoldHeader(foldedTxt):
    if foldedTxt: return foldedTxt.replace('\n',"""\n""")
...
# 
class DigestStream:
    def __init__(self, fmap, parser=email.parser.Parser(),unfold=True,decode=False):
        # the message file handle input stream
        self.inStream = fmap[0]
        self.fname = fmap[1]
	...
	msg = self.Parser.parse(self.inStream,headersonly=True)
        ...
        yield tuple([self.fname]+field_list)
    ...
    def fp_iterator(self):
        for info in self.tar:
            ...
            if info.isreg(): 
                ...
                yield (self.tar.extractfile(info.name),info.name)
...
def testParser():
    ...
    #m = map(lambda msgFile:DigestStream(msgFile,decode=False), MsgStreams('100Emails.tar.gz'))
    m = map(lambda msgFile:DigestStream(msgFile,parser=MicroParser(),decode=False), MsgStreams('100Emails.tar.gz'))
	...
	#print 'Date: %s\nSender: %s\nSubject: %s\n' % digest
        dig = [d or '' for d in digest]
	print '%s|%s|%s|%s' % (dig[0],dig[2],dig[3],dig[1])
